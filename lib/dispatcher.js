...
Dispatcher.prototype.doDispatchLoop = function(action) {
  ...
  _forOwn(this.waitingToDispatch, function(value, key) {
    dispatch = _this.currentDispatch[key];
    canBeDispatchedTo =
      !dispatch.waitingOn.length ||
      !_intersection(dispatch.waitingOn, _keys(_this.waitingToDispatch)).length;
    if (canBeDispatchedTo) {
      if (dispatch.waitCallback) {
        ...
        dispatch.resolved = true;
        var handled = _this.stores[key].__handleAction__(action);
        if (handled) {
          wasHandled = true;
        }
      }
      ...
      if (_this.currentDispatch[key].resolved) {
  ...
  _each(removeFromDispatchQueue, function(key) {
    delete _this.waitingToDispatch[key];
  });
  ...
  if (!wasHandled && console && console.warn) {
    console.warn(
      "An action of type " +
        action.type +
        " was dispatched, but no store handled it"
    );
  }
};
...
Dispatcher.prototype.waitForStores = function(store, stores, fn) {
  ...
  _each(stores, function(storeName) {
    var storeDispatch = _this.currentDispatch[storeName];
    if (!_this.stores[storeName]) {
